@extends('layouts.app', ['title' => 'Tambah Pengembalian'])

@section('content')
    <div class="card">
        <h2>Tambah Data Pengembalian</h2>
        <form action="{{ route('admin.returns.store') }}" method="post" class="grid">
            @csrf
            <div>
            ...
            <div class="grid grid-2">
                <div><label>Tgl Permintaan Kembali</label><input type="date" name="requested_return_date" value="{{ old('requested_return_date', now()->toDateString()) }}"></div>
                <div><label>Tgl Diterima</label><input type="date" id="received_date" name="received_date" value="{{ old('received_date') }}"></div>
                <div>
                    <label>Status</label>
                ...
                <div>
                    ...
                    <input type="text" id="condition_after_return" name="condition_after_return" value="{{ old('condition_after_return') }}" placeholder="Contoh: baik / rusak ringan / rusak berat">
                </div>
                <div>
                    <label>Denda</label>
                    <input type="number" id="fine" step="0.01" min="0" name="fine" value="{{ old('fine', 0) }}" readonly>
                    <p id="fine-info" class="muted" style="margin:6px 0 0;">Denda dihitung otomatis saat status verified.</p>
                </div>
            </div>
            <div><label>Catatan</label><textarea name="note">{{ old('note') }}</textarea></div>
            <div class="actions">
